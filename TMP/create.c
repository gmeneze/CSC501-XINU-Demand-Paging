...
void create_mapping(int frame_no); // To create PD for a process
...
/* Glen's code starts */
        ...
        get_frame_no = get_frm();

        frm_tab[get_frame_no].fr_type = FR_DIR; /* It's a page Dir */
        free_frame_no = FRAME0 + get_frame_no;

        create_mapping(free_frame_no);  // function in frame.c

        proctab[pid].pdbr = free_frame_no*NBPG; // setting PDBR for current process
